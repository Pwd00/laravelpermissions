                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <form action="{{route('roles.update',$role->id)}}" method="post">
                            ...
                            <div>
                                ...
                                <div class="my-2">
                                    <input type="text" placeholder="Enter roles" name="name" id="name" class="bg-grey-300 shadow-md w-1/2 rounded-lg text-black" value="{{old('name',$role->name)}}">
                                </div>
                                @error('name')
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-6 gap-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <input type="checkbox" id="perm-{{$per->id}}" class="form-checkbox h-5 w-5 text-blue-600 rounded-md" name="per[]" value="{{old('per',$per->name)}}" {{ ($hasPermission->contains($per->name)) ? 'checked' : '' }}>
