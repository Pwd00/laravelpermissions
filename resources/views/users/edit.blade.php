                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <form action="{{route('users.update',$user->id)}}" method="post">
                            ...
                            <div>
                                ...
                                <div class="my-2">
                                    <input type="text" placeholder="Enter Name" name="name" id="name" class="bg-grey-300 shadow-md w-1/2 rounded-lg text-black" value="{{old('name',$user->name)}}">
                                </div>
                                @error('name')
                                ...
                                <div class="my-2">
                                    <input type="email" placeholder="Enter Email" name="email" id="email" class="bg-grey-300 shadow-md w-1/2 rounded-lg text-black" value="{{old('email',$user->email)}}">
                                </div>
                                @error('email')
                                <div class="text-sm text-red-600 font-bold">{{$message}}</div>
                                @enderror
                                <div class="grid grid-cols-1 md:grid-cols-6 gap-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <input type="checkbox" id="role-{{$role->id}}" class="form-checkbox h-5 w-5 text-blue-600 rounded-md" name="role[]" value="{{old('role',$role->name)}}"  {{$hasRoles->contains($role->id)? 'checked': ''}}>
